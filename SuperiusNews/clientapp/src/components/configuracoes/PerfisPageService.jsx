...
export const PerfisPageService = () => {
    ...
    const gerenciarPerfis = async (codigoPerfil) => {
        setState(prev => ({ ...prev, loading: true }));
        const response = await apiConfig.perfil.axios.get(`${apiConfig.perfil.baseURL}${apiConfig.perfil.endpoints.permissoesAssociadasCompleto}/` + codigoPerfil);
        setState(prev => ({ ...prev, modal: { open: true, tipo: 'permissoes', data: response.data, chave: codigoPerfil}}));
        setState(prev => ({ ...prev, loading: false }));
    };

    const gerenciarPerfilConfirmar = async (codigoPerfil, arrCodigosPermissoes) => {
        setState(prev => ({ ...prev, loading: true }));
        try {
    ...
    return { ...state, editarPerfil, editarPerfilConfirmar, fecharModal, ativarPerfis, inativarPerfis, carregarPerfis, gerenciarPerfis, gerenciarPerfilConfirmar, clearMessages: () => setState(prev => ({ ...prev, error: false, success: false, mensagem: '' })) };
};